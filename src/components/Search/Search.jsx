...
class Search extends React.Component {
  ...
  handleButtonLine(cityName) {
    ...
    const ApiCall = {
      type: "NEW_CALL",
      payload: axios.get(
        `https://api.openweathermap.org/data/2.5/weather?q=${cityName}&APPID=dfeb2f059877c4edbbbeea5638365bd4&units=imperial`
      )
    };
  ...
  handleApiCall() {
    ...
    const ApiCall = {
      type: "NEW_CALL",
      payload: axios.get(
        `https://api.openweathermap.org/data/2.5/weather?q=${myInput}&APPID=dfeb2f059877c4edbbbeea5638365bd4&units=imperial`
      )
    };
    ...
    return (
      <div>
        <div>
          ...
          <button onClick={() => this.handleButtonLine("Washington")} className="button-between btn">Washington D.C</button>
